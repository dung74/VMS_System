...
async def start_camera(camera_id: int, data: CameraStartRequest):
    ...
    async with AsyncSessionLocal() as session:
        result = await session.execute(text("SELECT * FROM cameras WHERE id = :id"), {"id": camera_id})
        camera_record = result.fetchone()
    ...
    try:    
        ...
        print(f"[LỖI CAMERA] Source: {camera_id}, Error: {e}")
        traceback.print_exc() # Dòng này sẽ in ra chính xác lỗi nằm ở dòng code nào

        raise HTTPException(
            status_code=500, 
            detail=f"Failed to start camera {camera_id}. Error: {str(e)}"
        )
...
async def offer(camera_id: int, request: Request):
    print(f"==> [SYSTEM] Received WebRTC offer for camera {camera_id}")
